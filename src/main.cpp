...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		{
			recordMode = REPLAY_MODE::RECORD;
			replayData.filename = argv[++i];
		}
		else if (strcmp(argv[i], "-playback") == 0)
		{
			recordMode = REPLAY_MODE::PLAYBACK;
			replayData.filename = argv[++i];
		}
		else if (strcmp(argv[i], "-map") == 0)
		{
			if (argc > i + 1)
			{
				game.setMapName(argv[++i]);
				requiredSection = SECTION_GAME;
			}
		}
		else if (strcmp(argv[i], "-listmaps") == 0)
			listMaps();
			...
			hub = true;
		else if (strcmp(argv[i], "-randomscreens") == 0)
			graphics.takeRandomScreenShots = true;
